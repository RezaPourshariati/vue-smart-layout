...
import { register, RESET, sendVerificationEmail } from '@/redux/features/auth/authSlice.ts'
...
function Register() {
  ...
  const timesIcon: ReactNode = <FaTimes color="red" size={15} />
  const checkIcon: ReactNode = <BsCheck2All color="green" size={15} />
  ...
  const switchIcon = (condition: boolean): ReactNode => {
    if (condition)
      return checkIcon
    return timesIcon
  }
  ...
  useEffect(() => {
    if (password.length > 4 && !uppercase)
      setStyle('red')
    if (password.length > 4 && !num)
      setStyle('red')
    if (password.length > 4 && !specialCharacter)
      setStyle('red')
    if (password.length > 4 && !passLength)
      setStyle('red')
  ...
  const registerUser = async (e: FormEvent) => {
    ...
    if (password !== password2) {
      toast.error('Password wasn\'t match')
      return
    }
            ...
            <form onSubmit={registerUser}>
              ...
              <Card cardClass="border border-[#1f93ff] p-[5px] mb-[5px]">
                <ul className="form-list">
                  <li>
                    <span
                      className="flex justify-start items-center text-[10px]"
