...
import { RESET, resetPassword } from '@/redux/features/auth/authSlice.ts'
...
function Reset() {
  ...
  const { isLoading, isSuccess, message } = useSelector((state: RootState) => state.auth)
  ...
  const reset = (e: FormEvent) => {
    ...
    dispatch(resetPassword({ userData, resetToken: resetToken || '' }))
    navigate('/login')
  }

  useEffect(() => {
    if (isSuccess && message.includes('reset was successful'))
      navigate('/login')
    dispatch(RESET())
